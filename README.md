...
2.5. Вычисление постфиксного выражения
+ Используется стек для хранения промежуточных значений:
+ Числа и x → кладутся в стек;
+ Бинарные операторы → извлекают два операнда;
...
Печатается постфиксная форма;
Выводится итоговое значение или сообщение об ошибке.

## Контрольный расчёт
Входные данные:
Выражение cos(x-5)*10/(2*x-5)

Значение x = 5

Ожидаемый результат:
При подстановке x = 5 выражение принимает вид: 1 * 10 / 5 = 2
